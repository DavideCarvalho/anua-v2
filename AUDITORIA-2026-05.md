...
2. **Funil de pagamento responsável é frágil**: redirecionamento externo, sem comprovante automático, e _gating silencioso_ quando Asaas não está habilitado.
...
- **Esforço:** 1d.
- **Feito 2026-05-28 (sessão 6):** Spec + plano em `docs/superpowers/`. **Insight:** o gate de gamificação já é por idade (≤14), e autorresponsável é 18+ → já não-gamified; o gap real era acesso aos próprios dados. **Solução:** aluno autorresponsável vira seu próprio responsável via self-link em `StudentHasResponsible` (`studentId === responsibleId === user.id`), desbloqueando a checagem IDOR existente dos controllers `/responsavel` sem alterá-los. (a) `ensureSelfResponsibleLink` (idempotente) + hook na matrícula online (`finish_enrollment_controller`, dentro da transação) + comando `backfill:self-responsible-links`. (b) `resolveSelfResponsibleContext` expõe `isSelfResponsible`/`segment`/`studentId` no payload via `me.ts` (REST) e `inertia_middleware` (shared props) + tipo no frontend. (c) Defense-in-depth: `computeGamified` faz `&& !isSelfResponsible` (nunca gamified mesmo com birthDate errado). (d) Portal adulto em `/aluno` espelhando o `/responsável` escopado a ele: sidebar em 2 grupos — **Acadêmico** (Notas, Frequência, Horário, Atividades, Calendário, Ocorrências) e **Minha Conta** (Financeiro, Cantina, Documentos, Autorizações, Matrícula, Comunicados) — 12 páginas reusando os containers do responsável, gated por middleware `requireSelfResponsible`. Excluídos por princípio: Gamificação (autorresponsável nunca é gamified), Loja/Início (já no `/aluno`). Adiados: Chat/Perguntas (precisa extração + inquiry escopado por usuário) e Perfil (entidade diferente). **Verificação:** 17 testes funcionais passando (incl. acesso cruzado → 403), typecheck verde, revisão holística READY TO MERGE (IDOR sólido, paths de payload consistentes), e verificação visual ao vivo no Chrome (2 grupos + 12 itens, páginas renderizando com dados próprios, aluno comum sem grupos + redirecionado das rotas adultas). Threshold de idade/transição teen (#36) e Chat/Perfil ficaram fora de escopo.
