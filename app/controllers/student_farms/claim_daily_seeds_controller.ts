...
export default class ClaimDailySeedsController {
  async handle({ auth, effectiveUser, response }: HttpContext) {
    ...
    const farm = await studentFarmService.claimDailySeeds(student.id)
    return response.ok({
      farm: {
        seeds: farm.seeds,
        plots: farm.plots,
