...
export default class GetAttendanceOverviewController {
  async handle({ request, response }: HttpContext) {
    ...
    const [totalStudentsResult, attendanceStatsResult] = await Promise.all([
      ...
      // Stats de presenca. JUSTIFIED é status próprio no enum, contado direto.
      db.rawQuery(
        `
        SELECT
          ...
          COUNT(CASE WHEN sha.status = 'JUSTIFIED' THEN 1 END) as justified_count
    ...
    const justifiedCount = Number(attendanceStatsResult.rows[0]?.justified_count || 0)

    // Regra do domínio:
    // - presença = PRESENT + LATE (chegou, mesmo que tarde)
    // - falta = ABSENT + JUSTIFIED (justificar não tira a falta)
    const attendedCount = presentCount + lateCount
    const absencesCount = absentCount + justifiedCount
    const totalAbsences = absentCount + justifiedCount

    const attendanceRate = totalRecords > 0 ? (attendedCount / totalRecords) * 100 : 0
    const absentRate = totalRecords > 0 ? (absencesCount / totalRecords) * 100 : 0
    const lateRate = totalRecords > 0 ? (lateCount / totalRecords) * 100 : 0
    const justificationRate = totalAbsences > 0 ? (justifiedCount / totalAbsences) * 100 : 0
