...
import { useCanteenPurchasesQueryOptions } from '../hooks/queries/use-canteen-purchases'
...
// Error Fallback
function CanteenPurchasesError({
  error,
  resetErrorBoundary,
}: {
  error: Error
...
function CanteenPurchasesContent({
  ...
  const { data } = useSuspenseQuery(useCanteenPurchasesQueryOptions({ page, limit: 20 }))
              ...
              return (
                <tr key={purchase.id} className="border-t hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-4 font-semibold">
                    R$ {Number(purchase.total || 0).toFixed(2)}
                  </td>
                  <td className="p-4">
                    <span
        ...
        {({ reset }) => (
          <ErrorBoundary
            ...
            fallbackRender={({ error, resetErrorBoundary }) => (
              <CanteenPurchasesError error={error} resetErrorBoundary={resetErrorBoundary} />
            )}
          >
            <Suspense fallback={<CanteenPurchasesSkeleton />}>
              <CanteenPurchasesContent page={page} onPageChange={setPage} />
            </Suspense>
          </ErrorBoundary>
        )}
      </QueryErrorResetBoundary>
