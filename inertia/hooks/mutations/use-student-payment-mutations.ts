...
export function useMarkPaymentAsPaid() {
  ...
  return useMutation({
    mutationFn: ({ id, paidAt, paidAmount }: { id: string; paidAt?: string; paidAmount?: number }) => {
      return tuyau.$route('api.v1.studentPayments.markPaid', { id }).$post({ paidAt, paidAmount }).unwrap()
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['student-payments'] })
    },
