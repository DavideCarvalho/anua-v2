...
const $createRoute = tuyau.$route('api.v1.canteenItems.store')
type CreateCanteenItemPayload = InferRequestType<typeof $createRoute.$post>

const $updateRoute = tuyau.$route('api.v1.canteenItems.update')
type UpdateCanteenItemPayload = InferRequestType<typeof $updateRoute.$put> & { id: string }
...
export function useToggleCanteenItemActive() {
  ...
  return useMutation({
    mutationFn: (id: string) => {
      return tuyau.$route('api.v1.canteenItems.toggleActive', { id }).$post({}).unwrap()
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['canteen-items'] })
    },
