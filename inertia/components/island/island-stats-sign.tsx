...
interface IslandStatsSignProps {
  points: number
  level: number
  streak: number
}

export function IslandStatsSign({ points, level, streak }: IslandStatsSignProps) {
  const animatedPoints = useCountUp(points)
  const animatedLevel = useCountUp(level, 400)
  const animatedStreak = useCountUp(streak, 400)

  return (
    <div className="absolute left-1/2 -translate-x-1/2" style={{ top: '4%', zIndex: 30 }}>
