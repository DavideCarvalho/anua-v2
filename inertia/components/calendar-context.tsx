...
export function CalendarProvider({
  ...
  const applyEventFilters = (
    ...
    let nextEvents = sourceEvents

    if (userId !== 'all') {
      nextEvents = nextEvents.filter((event) => event.user.id === userId)
    }

    if (colors.length > 0) {
      nextEvents = nextEvents.filter((event) =>
        colors.includes((event.color || 'blue') as TEventColor)
      )
    }
  ...
  const filterEventsBySelectedColors = (color: TEventColor) => {
    ...
    setFilteredEvents(applyEventFilters(allEvents, selectedUserId, newColors))
  ...
  const filterEventsBySelectedUser = (userId: IUser['id'] | 'all') => {
    ...
    setFilteredEvents(applyEventFilters(allEvents, userId, selectedColors))
  }
  ...
  useEffect(() => {
    setAllEvents(events || [])
    setFilteredEvents(applyEventFilters(events || [], selectedUserId, selectedColors))
