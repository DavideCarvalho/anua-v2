...
import { formatTime, getEventBlockHeight } from '~/components/helpers'
import type { IEvent } from '~/components/interfaces'

const calendarWeekEventCardVariants = cva(
  'flex min-w-0 select-none flex-col gap-0.5 overflow-hidden truncate whitespace-nowrap rounded-md border px-2 py-1.5 text-xs focus-visible:outline-offset-2',
  {
    variants: {
      color: {
        // Colored variants
...
export function EventBlock({ event, className }: IProps) {
  ...
  const heightInPixels = getEventBlockHeight(event)
