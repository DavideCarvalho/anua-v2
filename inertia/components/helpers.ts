...
export function getEventBlockHeight(event: IEvent): number {
  const durationInMinutes = differenceInMinutes(parseISO(event.endDate), parseISO(event.startDate))
  return (durationInMinutes / 60) * 96 - 8
}
